...
def TET_loss(result, target):
    loss = 0.
    for t in range(result.shape[0]):
        loss += F.cross_entropy(result[t], target)
    return loss / result.shape[0]
...
class VGG8(nn.Module):
    def __init__(self):
        ...
        self.block2 = nn.Sequential(
            ...
            SMA(channel=128, C_reduction=4, time=14, T_reduction=4, AZO=True, RCR=24, RTR=4),
        ...
        self.block3 = nn.Sequential(
            ...
            SMA(channel=256, C_reduction=4, time=14,  T_reduction=4, AZO=True, RCR=24, RTR=4),
        ...
        self.others = nn.Sequential(
            ...
            neuron.LIFNode(v_threshold=1.0, v_reset=0.0, detach_reset=True, tau=2.0,
                           surrogate_function=surrogate.ATan()),
            layer.Dropout(p=0.5),
            layer.Linear(in_features=1024, out_features=101, bias=True)
        )
...
parser = argparse.ArgumentParser(description='SMA-VGG N-Caltech101 Training')
...
parser.add_argument('--num-workers', default=70, type=int, help='Number of CPU cores used for loading datasets\n')
parser.add_argument('-b', '--batch-size', default=6, type=int, help='Batch size, e.g., "64"\n')
parser.add_argument('-T', '--timesteps', default=14, type=int, dest='T', help='Simulation duration, e.g., "100"\n')
parser.add_argument('--lr', '--learning-rate', default=1e-3, type=float, metavar='LR', help='Learning rate, e.g., "1e-3"\n',
                    dest='lr')
parser.add_argument('-N', '--epoch', default=300, type=int, help='Training epochs\n')
parser.add_argument('--decay', default=300, type=int, help='Learning rate decay times\n')
parser.add_argument('--local-rank', default=-1, type=int, help='Multi-card training, indicates the number of GPUs (processes)') 
...
def main():
    ...
    dist.init_process_group(backend='nccl')  # nccl is the fastest and most recommended backend for GPU devices
    ...
    spiking_rate = []  # For plotting the frequency histogram of neuron firing rates
    test_loss_list = []
    ...
    train_dataset = NCaltech101(root='../data/NCaltech101/train_dataset', data_type='frame', frames_number=num_steps, split_by='number')
    test_dataset = NCaltech101(root='../data/NCaltech101/test_dataset', data_type='frame', frames_number=num_steps, split_by='number')
    ...
    print(f"Number of samples in training set: {len(train_dataset)}, Number of samples in test set: {len(test_dataset)}")

    net = Net
    print(net)

    # Build the model
    # DDP: Loading the model should be done before constructing the DDP model, and it only needs to be done on the master.
    net = net.to(local_rank)
    # net = nn.SyncBatchNorm.convert_sync_batchnorm(net)  # Synchronize batch normalization
    net = DDP(net, device_ids=[local_rank], output_device=local_rank)

    optimizer = torch.optim.NAdam(net.parameters(), lr=lr, momentum_decay=1e-3)
    ...
    for epoch in range(1, epochs + 1):
        ...
        # DistributedSampler needs this to specify the shuffle method,
        # by maintaining the same random seed among processes to achieve the same shuffle effect.
        train_loader.sampler.set_epoch(epoch)
        test_loader.sampler.set_epoch(epoch)

        train_correct_sum_in_epoch = 0        # Number of correct predictions in the training data for each epoch
        train_data_sum_in_epoch = 0           # Total amount of data used for training in each epoch
        ...
        for img, label in tqdm(train_loader):                       # Number of iterations ≈ Total data / batch_size
            img = img.to(local_rank)
            img = img.transpose(0, 1)
            # print(img.shape)                                        # [T, B, C, h, w]
            label = label.to(local_rank)
            label_one_hot = F.one_hot(label, 101).float()            # One-hot encode the labels for calculating the loss later
            # Mixed precision training
            with amp.autocast():
                result_has_t = net(img)
                result = result_has_t.mean(0)
                ...
                loss = TET_loss(result_has_t, label_one_hot)  # MSE loss
                optimizer.zero_grad()
                scaler.scale(loss).backward()
                scaler.step(optimizer)
                scaler.update()
            functional.reset_net(net)  # Reset network state
            train_data_sum_in_epoch += label.numel()  # numel function returns the number of elements in the array, which is the total number of images
        # Sum up the correct number and total number of data across threads
        ...
        if dist.get_rank() == 0:
            ...
            writer.add_scalars('train_and_test_accuracy', {'train': train_accuracy_in_epoch}, epoch)  # Store accuracy for each epoch in tensorboard
            writer.add_scalar('loss_epoch', loss.item(), epoch)  # Store loss
        ...
        with torch.no_grad():  # Test once for each epoch
            test_data_correct_sum = 0  # Number of correct model outputs during testing
            test_data_sum = 0  # Total amount of data during testing
            for img, label in tqdm(test_loader):
                ...
                result_has_t = net(img)
                result = result_has_t.mean(0)
                label_one_hot_test = F.one_hot(label, 101).float()  # One-hot encode the labels for calculating the loss later
                loss_test = TET_loss(result_has_t, label_one_hot_test)
                # Calculate the number of correct classifications
                pred = result.argmax(dim=1)
                correct = pred.eq(label).sum().float().item()
                test_data_correct_sum += correct  # Accumulate the number of correct predictions
                test_data_sum += label.numel()  # Accumulate the total amount of data
        ...
        if dist.get_rank() == 0:
            ...
            writer.add_scalars('train_and_test_accuracy', {'test': test_accuracy}, epoch)  # Store test accuracy
            test_accs.append(test_accuracy)  # Store test accuracy
            ...
            print(f"epoch{epoch}中，训练集准确率为{train_accuracy_in_epoch}, 测试准确率为{test_accuracy},至今最好的测试准确率为{max_test_accuracy}")
            ...
            # Model saving seems not to coexist with the hook created when saving the spiking rate
            # if save_max:
    ...
    if dist.get_rank() == 0:
        ...
        np.save(model_output_dir + '/' + filename + '_test_acc.npy', test_accs)  # Save the accuracy list for each epoch on the test set in binary file format
        ...
        np.save(model_output_dir + '/' + filename + '_test_loss.npy', test_loss_list)  # Save the loss list for each epoch in binary file format
